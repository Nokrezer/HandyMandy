...
export function UserPostsBlock({userInfo, isUserPage})//Компонент, загрузка постов пользователя
{
    ...
    //Подгрузка постов
    useEffect(() => {
        if (!loadMorePosts.current) return;
        
        const handleScroll = async () => {
            if (isLoadingPosts.current) return;
    ...
    return (<div id="posts">
                ...
                {posts.map(post => 
                    <Post key={post.post_id} userInfo={userInfo} post={post} fileToken={fileToken} isUserPage={isUserPage} setPosts={setPosts}/>
                )}
            </div>);
}
...
function FeedPostsBlock(){
    ...
    return (<div id="posts">
                ...
                {posts.map(post => 
                    <Post key={post.id} userInfo={userInfo} post={post} fileToken={fileToken} isUserPage={isUserPage} posts={posts} setPosts={setPosts}/>
                )}
            </div>);;
}