...
export default function UserMenuBarComponent({userSettings})//Компонент, Меню
{ 
    ...
    useEffect(() => {(async () => {
            try{
                ...
                const menuItems = [//url - перенаправление по нажатию на кнопку, image - ссылка на изображение
    ...
    return (
        <div id="menu">
        {
            menuData.map(buttonInfo => <Link to={buttonInfo.url} className="menu-button" key={buttonInfo.text}>
                {buttonInfo.image.includes("svg") ? <div className="menu-button-img"><object data={CONFIG.SITE_IP + buttonInfo.image} type="image/svg+xml"/></div> : <img className="menu-button-img" src={buttonInfo.image}/>}
                <h2 className="menu-button-text">{buttonInfo.text}</h2>
            </Link>)
        }
    </div>);
}