...
export class AuthService{
    ...
    async login(formData){//отправка данных авторизации, и возвращаем access token
    ...
    async registration(formData)
    {
        ...
        await this.sendRequest(CONFIG.SITE_IP + AUTH.registration, {//Делаем запрос к серверу
            method:"POST",
            body: data
        });
    }

    async verifyToken(accessToken)//Метод для проверки годности токена, False - негоден, True - годен
    {
        if(!accessToken)
            return false;
    ...
    async getAccessToken()//Метод для обновления refresh токена и получение access токена
    {
